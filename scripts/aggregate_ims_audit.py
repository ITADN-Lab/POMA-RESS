"""Aggregate NASA IMS Bearing audit (Plan B v3 RESS-native external)."""
...
def load_runs(res_dir, opt, sp):
    runs = []
    for f in glob.glob(os.path.join(res_dir, f'ims_{opt}_seed*_*.json')):
        try:
            d = json.load(open(f))
...
def main():
    ...
    ap.add_argument('--res_dir', default=os.path.expanduser('~/pmo_ims/results'))
    args = ap.parse_args()

    md = ["# NASA IMS Bearing audit panel (Plan B v3 RESS-native)\n",
          "Cross-source bearing benchmark (NASA Ames IMS, 12 bearings × 3 test sets, 20kHz vibration). "
          "4 candidates × 2 partitions × 3 seeds at each partition's best 12-config grid.\n",
    ...
    md.append("| Opt | sp=2024 | sp=7 |"); md.append("|---|---|---|")
    for opt in CANDIDATES:
        d = summary.get(opt, {}); row = [opt]
        for sp in PARTITIONS:
            x = d.get(sp)
        ...
        md.append("| " + " | ".join(row) + " |")
    out_md = os.path.join(args.res_dir, 'IMS_AUDIT_ANALYSIS.md')
    open(out_md, 'w').write("\n".join(md) + "\n")
    json.dump(summary, open(os.path.join(args.res_dir, 'ims_summary.json'), 'w'), indent=2)
    print("\n".join(md)); print(f"\nwrote {out_md}")
