"""
NASA Battery audit runner — minimum-cost external replication (Plan B v2).

Codex spec: 4 optimizers × 2 partitions × 3 seeds per partition's best
config + a small 12-config Phase A grid per optimizer per partition.
With 4 batteries (B0005/B0006/B0007/B0018) and 25%/25%/50% split, each
partition holds out 1 test battery — leave-one-out CV across all 4
batteries.
"""
...
TRAIN = os.path.join(THIS, 'train_battery_leakfree.py')

OPTS = ['AdamW', 'PMO', 'Adan', 'RAdam', 'Lion']
# minimal Phase A grid (12 configs/opt) — smaller than C-MAPSS/FEMTO
# given dataset size (~150 cycles/battery).
BETA1_PER_OPT = {
...
PARTITIONS = [2024, 7]
SEEDS_B = [42, 123, 456]   # 3-seed minimal replication (Codex spec)
MAX_RETRY = 3
...
def out_name(opt, b1, gc, lr, seed, sp):
    ...
    return f"battery_{opt}_seed{seed}_{tag}{sp_s}.json"
...
def collect_best(save_dir, opt, sp, seed=42):
    best = None
    for f in glob.glob(os.path.join(save_dir, f'battery_{opt}_seed{seed}_*.json')):
        try:
            d = json.load(open(f))
