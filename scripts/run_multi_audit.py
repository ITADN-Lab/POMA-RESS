"""
Plan B (Codex gpt-5.5 top priority): multi-optimizer audit panel.
Apply the same leak-free engine-level equal-budget protocol to RAdam and Lion.
AdamW + PMO + Adan results already in results_leakfree/.
"""
...
OPTIMIZERS = ['RAdam', 'Lion']
SUBSETS = ['FD001', 'FD002', 'FD003', 'FD004']
BETA1_PER_OPT = {
    'RAdam': [0.8, 0.9, 0.95],
    'Lion':  [0.9, 0.95, 0.99],     # Lion's recommended β1 range
}
GC_GRID = [0.0, 0.5, 1.0, 2.0]
LR_PER_OPT = {
    'RAdam': [3e-4, 1e-3, 3e-3],
    'Lion':  [1e-4, 3e-4, 1e-3],    # Lion typically uses smaller LR than AdamW
}
MAIN_PARTITION = 2024
ALT_PARTITIONS = [7, 99]
...
def main():
    ...
    if 'A' in args.phase:
        ...
        launch(jobs, args.save_dir, args, f'Phase A ({len(OPTIMIZERS)} opt grid)')
